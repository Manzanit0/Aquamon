...
namespace Aquamon.Commands.Sandboxes
{
    public abstract class SandboxCommand : ICommand
    {
        ...
        protected static SandboxInfo CreateSandboxInfo()
        {
            ...
            var configurator =
                new Configurator<SalesforceConfiguration>(); // TODO - verify if it's possible to get just the SandboxInfo config without traversing the whole thing.
        ...
        protected static SandboxInfo CreateSandboxInfo(CommandLineApplication command)
        {
            ...
            sbxInfo.Description = command.Options.Where(x => x.ShortName == "d").FirstOrDefault().ValueName ??
                                  "Default Description.";
            sbxInfo.ApexClassId = command.Options.Where(x => x.ShortName == "a").FirstOrDefault().ValueName ??
                                  sbxInfo.ApexClassId; // Override if forced via console parameter.
